...
export default function Home() {
  ...
  const handleSubmit = async () => {
    try {
      ...
      const res = await axios.post(
        "http://localhost:8000/api/chatQuery/intent",
        { expense, mode: "chat" }
      );
      // const res = await axios.post(
      //   "http://localhost:8000/api/bulkAdd/bulkAdd",
      //   { expense }
      // );
  ...
  return (
    <section className="m-10 flex flex-col gap-4">
      <Label htmlFor="expense" className="text-[#e6e0e0] font-bold">
        Enter Your Expense :
      </Label>
      <Input
        id="expense"
      ...
      {
        <div
          className="text-[#e6e0e0] font-bold"
